          ...
          {services.map((service) => (
            <Card key={service.title} className="overflow-hidden border-2 hover:border-winter-blue/30 transition-all duration-300 group shadow-card hover:shadow-winter hover:-translate-y-2 animate-fade-in">
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-winter-blue/40 group-hover:bg-gradient-to-b group-hover:from-transparent group-hover:to-winter-blue/60 transition-all duration-300" />
                
                {/* Title positioned over image */}
                <div className="absolute bottom-4 left-4 right-4 z-10">
                  <h3 className="text-2xl font-bold text-white drop-shadow-lg group-hover:text-[26px] group-hover:drop-shadow-2xl transition-all duration-300" style={{textShadow: 'none'}} onMouseEnter={(e) => e.currentTarget.style.textShadow = '2px 2px 8px rgba(0, 0, 0, 0.8)'} onMouseLeave={(e) => e.currentTarget.style.textShadow = 'none'}>
                    {service.title}
                  </h3>
                </div>
              </div>
